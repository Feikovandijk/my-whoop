...
fun getEnvValue(key: String, defaultValue: String): String {
    ...
    val serverEnv = File(rootDir, "../server/.env")
    if (serverEnv.exists()) {
        val props = Properties()
        serverEnv.inputStream().use { props.load(it) }
    ...
    if (rootEnv.exists()) {
        val props = Properties()
        rootEnv.inputStream().use { props.load(it) }
        val propVal = props.getProperty(key)
        if (!propVal.isNullOrEmpty()) return propVal
    }
...
val serverUrl = getEnvValue("WHOOP_SERVER_URL", "http://10.0.2.2:8770")
val apiKey = getEnvValue("WHOOP_API_KEY", "")
...
android {
    ...
    defaultConfig {
        ...
        buildConfigField("String", "DEFAULT_API_KEY", "\"$apiKey\"")
